...
class BrowserManager:
    ...
    async def goto(
        ...
        try:
            ...
            # Some authenticated HMS routes can trigger a client-side
            # navigation/reload that Playwright reports as ERR_ABORTED.
            # If the browser has already reached a usable document,
            # continue instead of treating the navigation as a hard failure.
            message = str(exc)
            ...
            logger.warning(
                "Navigation reported ERR_ABORTED for %s; "
                "checking the current document instead.",
                url,
            )
            ...
            try:
                ...
                if not current_url or current_url == "about:blank":
                    raise RuntimeError(
                        f"Navigation aborted and no usable page loaded: {url}"
                    )

                logger.info(
                    "Continuing after aborted navigation: "
    ...
    async def screenshot(
        ...
        """Capture the current page for QA evidence."""
        assert self.page is not None
