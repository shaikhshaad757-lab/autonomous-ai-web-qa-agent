...
class TestingConfig(BaseModel):
    ...
    # Controlled synthetic-data testing.
    # This is intentionally separate from general mutation testing.
    allow_test_data_mutations: bool = False
...
class LLMConfig(BaseModel):
    ...
    max_tokens: int = 1024
    # Read timeout for a single Ollama request. Previously 60s with 2
    # hardcoded attempts meant up to 120s per bug, 20 minutes worst case
    # across the 10-bug analysis cap. 20s keeps a local model's normal
    # response time comfortable while bounding the worst case tightly.
    request_timeout_seconds: int = 20
    # Separate, shorter timeout for establishing the connection itself
    # (distinct from waiting for the model to finish generating).
    connect_timeout_seconds: float = 5.0
    # Additional attempts beyond the first (1 => 2 total attempts,
    # matching previous hardcoded behavior, but now configurable).
    max_retries: int = 1
